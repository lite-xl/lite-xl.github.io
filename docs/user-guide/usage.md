...
— this provides a fuzzy finder over all the project's files and can be
...
The additional files will be also visible when searching across the project.
...
remember your workspace including the additional project directories.
...
* **Predicate** — A function that returns true if the command can be run, for
...
Commands can be performed programmatically (e.g. from another command or by your
...
- `ARGS`: command-line arguments.
  `argv[1]` is the program name, `argv[2]` is the 1st parameter, ...
- `PLATFORM`: Output from `SDL_GetPlatform()`.
  Can be `Windows`, `Mac OS X`, `Linux`, `iOS` and `Android`.
- `SCALE`: Font scale.
  Usually 1, but can be higher on Hi DPI systems.
- `EXEFILE`: An absolute path to the executable.
- `EXEDIR`: The executable directory.
  **DO NOT WRITE TO THIS DIRECTORY.**
- `VERSION`: Lite XL version.
- `MOD_VERSION`: mod-version used in plugins.
  This is usually incremented when there are API changes.
- `PATHSEP`: Path separator. `\` (Windows) or `/` (Other OSes)
- `DATADIR`: The data directory, where the Lua part of Lite XL resides.
  **DO NOT WRITE TO THIS DIRECTORY.**
...
> (for example, if the editor is installed in `/usr`,
> `DATADIR` will be `/usr/share/lite-xl`!)
> `USERDIR` on the other hand should always be writable for the user,
> and allows multiple users to customize their own editor.
...
and will then load the plugins located in the user module directory.
...
Plugins can be downloaded from the [plugin repository][2].
...
Color themes in Lite XL are Lua modules which overwrite the color fields of
...
Color themes can be downloaded from the [color theme repository][3].
